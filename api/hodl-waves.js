...
// bitcoin-data.com), but this endpoint returns supply-by-age-band
// percentages per date rather than a single value, so each row keeps
// every numeric band field found instead of picking just one.
...
// The exact band field names aren't hardcoded — whatever numeric,
// non-date/id fields a row has are kept as-is and treated as age bands.
// The frontend discovers band keys from the data itself.
function normalizeRow(row) {
  ...
  const bands = {};
  keys.forEach((k) => {
    if (k === dateKey || NON_BAND_KEY.test(k) || !isNumeric(row[k])) return;
...
// ---- Persistent archive (Upstash Redis), same pattern as api/onchain-metrics.js ----
const REDIS_URL = process.env.UPSTASH_REDIS_REST_URL;
const REDIS_TOKEN = process.env.UPSTASH_REDIS_REST_TOKEN;
const ARCHIVE_KEY = "hodl-waves-archive";
...
export default async function handler(req, res) {
  ...
  if (!history.length) {
    res.status(502).json({ error: "hodl-waves-supply: no parseable records", debug: fetchDebug });
    return;
  }
